...
class CajaManager(models.Manager):
    ...
    def report_by_date(self, date1, date2, sucursal):
        ...
        if len(date1) > 0 and len(date2) and len(sucursal) > 0:
            ...
            print(sucursal)
            reportes = self.filter(Q(created__gte = fecha1)&Q(created__lte = fecha2), nombre_caja__icontains = sucursal).order_by('-created')
            ...
            reportes = self.filter(nombre_caja__icontains = sucursal)
        else:
